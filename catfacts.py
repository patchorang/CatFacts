...
class CatFacts:
	...
	def __init__(self, username, apppass, auth_token, rnr_se, number, minutes_per_text = 60, audio_alert = False):
		self.promotional_messages = [
			...
			"To unsubscribe at anytime reply STOP.",
			"To unsubscribe at anytime reply PLEASE STOP.",
			"To unsubscribe at anytime reply i HaTe CaTs.",
			"To unsubscribe at anytime reply CATZ.",
			"To unsubscribe at anytime reply I LOVE CATS."	
		]
		self.fact_list = """Every year, nearly four million cats are eaten in Asia.
...
The first cat in space was a French cat named Felicette (a... "Astrocat") In 1963, France blasted the cat into outer space. Electrodes implanted in her brains sent neurological signals back to Earth. She survived the trip.
...
If they have ample water, cats can tolerate temperatures up to 133F.
...
The richest cat is Blackie who was left $20 million by his owner, Ben Rea.
...
The claws on the cat's back paws aren't as sharp as the claws on the front paws because the claws in the back don't retract and, consequently, become worn.""".split("\n\n")
	...
	def getRandomPromo(self):
		return self.promotional_messages[randrange(len(self.promotional_messages))]
